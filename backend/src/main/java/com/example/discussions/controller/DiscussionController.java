...
public class DiscussionController {
  ...
  public DiscussionController(DiscussionRepository d, CategoryRepository c, CommentRepository comments,
      VoteRepository v, CommentVoteRepository commentVotes, ReactionRepository reactions, CurrentUser current) {
    this.discussions=d; this.categories=c; this.comments=comments; this.votes=v;
    this.commentVotes=commentVotes; this.reactions=reactions; this.current=current;
  }
  ...
  private DiscussionView view(Discussion d){var u=current.optional();var roots=d.comments.stream().filter(c->c.parent==null).map(this::commentView).toList();return new DiscussionView(d.id,d.title,d.body,d.status.name(),d.author,d.category,votes.countByDiscussion(d),d.comments.size(),u.flatMap(x->votes.findByUserAndDiscussion(x,d)).isPresent(),reactionViews(reactions.findByDiscussion(d)),d.createdAt,d.updatedAt,roots);}
