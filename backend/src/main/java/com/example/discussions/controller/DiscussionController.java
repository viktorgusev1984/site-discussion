...
import com.example.discussions.service.notification.NotificationEvent;
...
public class DiscussionController {
  ...
  @PostMapping("/discussions") @ResponseStatus(HttpStatus.CREATED) public DiscussionView create(@Valid @RequestBody DiscussionInput in){var d=new Discussion();d.author=current.required();apply(d,in);discussions.saveAndFlush(d);notifications.publishNewDiscussion(d.author,d,d.createdAt);return view(d);}
  @PutMapping("/discussions/{id}") public DiscussionView update(@PathVariable Long id,@Valid @RequestBody DiscussionInput in){var d=find(id);current.ownerOrAdmin(d.author);apply(d,in);touch(d);return view(d);}
  ...
  @PostMapping("/discussions/{id}/comments") @ResponseStatus(HttpStatus.CREATED) public Comment comment(@PathVariable Long id,@Valid @RequestBody CommentInput in){var c=new Comment();c.discussion=find(id);c.author=current.required();c.body=in.body();if(in.parentId()!=null)c.parent=comments.findById(in.parentId()).filter(x->x.discussion.id.equals(id)).orElseThrow(()->ApiException.notFound("Родительский комментарий не найден"));var at=touch(c.discussion);comments.saveAndFlush(c);var recipient=c.parent==null?c.discussion.author:c.parent.author;publish(c.parent==null?NotificationTrigger.NEW_COMMENT:NotificationTrigger.NEW_REPLY,c.author,recipient,c.discussion,c.id,at,null,null,null,"comment-created:"+c.id);return c;}
  @PutMapping("/discussions/{id}/vote") public DiscussionView vote(@PathVariable Long id){var d=find(id);var u=current.required();if(votes.findByUserAndDiscussion(u,d).isEmpty()){var v=new Vote();v.user=u;v.discussion=d;votes.saveAndFlush(v);var at=touch(d);publish(NotificationTrigger.FIRST_VOTE,u,d.author,d,null,at,null,null,null,"discussion-vote:"+v.id);}return view(d);}
  ...
  @PutMapping("/comments/{id}/vote") public CommentView voteComment(@PathVariable Long id){var c=findComment(id);var u=current.required();if(commentVotes.findByUserAndComment(u,c).isEmpty()){var v=new CommentVote();v.user=u;v.comment=c;commentVotes.saveAndFlush(v);var at=touch(c.discussion);publish(NotificationTrigger.FIRST_VOTE,u,c.author,c.discussion,c.id,at,null,null,null,"comment-vote:"+v.id);}return commentView(c);}
  ...
  @PutMapping("/discussions/{id}/close") public DiscussionView close(@PathVariable Long id){var d=find(id);var actor=current.moderator();changeStatus(d,Discussion.Status.CLOSED,actor);return view(d);}
  @PutMapping("/discussions/{id}/cancel") public DiscussionView cancel(@PathVariable Long id){var d=find(id);var actor=current.ownerOrModerator(d.author);changeStatus(d,Discussion.Status.CANCELLED,actor);return view(d);}
  @PostMapping("/discussions/{id}/actions/jira") @ResponseStatus(HttpStatus.CREATED) public DiscussionView addJira(@PathVariable Long id,@Valid @RequestBody JiraActionInput in){var d=find(id);var actor=current.moderator();var action=new DiscussionAction();action.discussion=d;action.actor=actor;action.type=DiscussionAction.Type.JIRA;action.label=in.key();action.url=in.url();actions.saveAndFlush(action);d.actions.add(action);var at=touch(d);publish(NotificationTrigger.JIRA_ACTION_CREATED,actor,d.author,d,null,at,null,null,action.label,"jira-action:"+action.id);return view(d);}
  @PutMapping("/comments/{id}/reactions/{emoji}") public CommentView reactComment(@PathVariable Long id,@PathVariable String emoji){var c=findComment(id);validateEmoji(emoji);var u=current.required();if(reactions.findByUserAndCommentAndEmoji(u,c,emoji).isEmpty()){var r=new Reaction();r.user=u;r.comment=c;r.emoji=emoji;reactions.saveAndFlush(r);var at=touch(c.discussion);publish(NotificationTrigger.NEW_REACTION,u,c.author,c.discussion,c.id,at,null,emoji,null,"comment-reaction:"+r.id);}return commentView(c);}
  ...
  private Instant touch(Discussion d){var at=Instant.now();d.updatedAt=at;discussions.save(d);return at;}
  private void changeStatus(Discussion d,Discussion.Status status,User actor){if(d.status==status)return;d.status=status;var at=touch(d);publish(NotificationTrigger.STATUS_CHANGED,actor,d.author,d,null,at,status.name(),null,null,"discussion-status:"+d.id+":"+status);}
  private void publish(NotificationTrigger trigger,User actor,User recipient,Discussion d,Long commentId,Instant at,String status,String emoji,String actionLabel,String eventId){notifications.publish(new NotificationEvent(eventId,trigger,actor.id,recipient.id,d.id,commentId,at,new NotificationEvent.TemplateData(d.title,actor.displayName,status,emoji,actionLabel)),d);}
}
