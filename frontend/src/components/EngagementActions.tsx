...
export default function EngagementActions({id,target,count,active=false,initialReactions=[]}:{id:number;target:'discussions'|'comments';count:number;active?:boolean;initialReactions?:Reaction[]}){
  ...
  return <div className="engagement" ref={root}>
    <button type="button" className={`compact-vote ${voted?'active':''}`} aria-label={!user?'Войдите, чтобы проголосовать':voted?'Снять голос':'Проголосовать'} aria-pressed={voted} disabled={busy||!user} onClick={toggleVote}><span aria-hidden="true">↑</span><b>{votes}</b></button>
    {items.map(item=><button type="button" key={item.emoji} className={`reaction ${item.reactedByMe?'active':''}`} aria-pressed={item.reactedByMe} disabled={!user} onClick={()=>toggleReaction(item.emoji)}><span>{item.emoji}</span><b>{item.count}</b></button>)}
    {user&&<div className="reaction-picker-wrap"><button type="button" className="add-reaction" aria-label="Добавить реакцию" aria-expanded={open} onClick={()=>setOpen(value=>!value)}>☺<span className="plus">+</span></button>{open&&<div className="reaction-picker" role="menu" aria-label="Выберите реакцию">{EMOJIS.map(emoji=><button type="button" role="menuitem" key={emoji} onClick={()=>toggleReaction(emoji)}>{emoji}</button>)}</div>}</div>}
  </div>
}
