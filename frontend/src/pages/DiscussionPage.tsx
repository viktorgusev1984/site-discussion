...
export default function DiscussionPage(){
  ...
  const moderator=user?.role==='MODERATOR'||user?.role==='ADMIN',author=user?.id===item.author.id;
  ...
  return <main className="detail"><Link to="/">← Все обсуждения</Link><div className="detail-grid"><article>
    ...
    <h1>{item.title}</h1><div className="byline"><span className="avatar">{item.author.displayName[0]}</span>{item.author.displayName} · {new Date(item.createdAt).toLocaleDateString('ru')} {author&&<Link to={`/discussions/${id}/edit`}>Редактировать</Link>}</div>
    {(moderator||author)&&item.status!=='CLOSED'&&item.status!=='CANCELLED'&&<div className="discussion-controls">{moderator&&<button className="primary" onClick={()=>transition('close')}>Закрыть обсуждение</button>}<button className="secondary danger" onClick={()=>transition('cancel')}>Отменить обсуждение</button>{moderator&&<button className="secondary" onClick={()=>setJiraOpen(open=>!open)}>Добавить задачу Jira</button>}</div>}
