...
export const discussionsApi={list:(filters:Filters)=>api.get<Page<Discussion>>('/discussions',{params:filters}).then(r=>r.data),get:(id:string|number)=>api.get<Discussion>(`/discussions/${id}`).then(r=>r.data),save:(data:Partial<Discussion>,id?:string)=>api.request<Discussion>({url:id?`/discussions/${id}`:'/discussions',method:id?'put':'post',data}).then(r=>r.data),remove:(id:number)=>api.delete(`/discussions/${id}`),vote:(id:number,active:boolean)=>api.request({url:`/discussions/${id}/vote`,method:active?'put':'delete'}).then(r=>r.data),comment:(id:number,body:string,parentId?:number)=>api.post(`/discussions/${id}/comments`,{body,parentId}).then(r=>r.data),categories:()=>api.get<Category[]>('/categories').then(r=>r.data)};
